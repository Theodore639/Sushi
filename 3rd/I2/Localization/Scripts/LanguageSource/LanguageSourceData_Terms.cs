    ...
    public partial class LanguageSourceData
    {
        ...
        public void UpdateDictionary(bool force = false)
        {
            ...
            StringComparer comparer =
                (CaseInsensitiveTerms ? StringComparer.OrdinalIgnoreCase : StringComparer.Ordinal);
        ...
        public string GetTranslation(string term, string overrideLanguage = null, string overrideSpecialization = null,
            bool skipDisabled = false, bool allowCategoryMistmatch = false)
        {
            ...
            if (TryGetTranslation(term, out Translation, overrideLanguage: overrideLanguage,
                overrideSpecialization: overrideSpecialization, skipDisabled: skipDisabled,
                allowCategoryMistmatch: allowCategoryMistmatch))
                return Translation;
        ...
        public bool TryGetTranslation(string term, out string Translation, string overrideLanguage = null,
            string overrideSpecialization = null, bool skipDisabled = false, bool allowCategoryMistmatch = false)
        {
            int Index = GetLanguageIndex(
                overrideLanguage == null ? LocalizationManager.CurrentLanguage : overrideLanguage, SkipDisabled: false);
            ...
            if (Index >= 0 && (!skipDisabled || mLanguages[Index].IsEnabled()))
            {
                ...
                else if (OnMissingTranslation == MissingTranslationAction.Fallback && data != null)
                {
                    return TryGetFallbackTranslation(data, out Translation, Index, overrideSpecialization,
                        skipDisabled);
                }
                ...
                else if (OnMissingTranslation == MissingTranslationAction.ShowTerm)
                {
                    Translation = term;
                    return true;
                }
        ...
        bool TryGetFallbackTranslation(TermData termData, out string Translation, int langIndex,
            string overrideSpecialization = null, bool skipDisabled = false)
        {
        ...
        public void ClearTerm(string term)
        {
            for (int i = 0, imax = mTerms.Count; i < imax; ++i)
                if (mTerms[i].Term == term)
                {
                    for (int j = 1; j < mTerms[i].Languages.Length; j++)
                    {
                        mTerms[i].Languages[j] = string.Empty;
                    }
