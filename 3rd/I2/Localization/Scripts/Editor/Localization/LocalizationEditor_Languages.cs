    ...
    public partial class LocalizationEditor
    {
        ...
        void OnGUI_Languages()
        {
            ...
            mProp_IgnoreDeviceLanguage.boolValue = EditorGUILayout.Popup(mProp_IgnoreDeviceLanguage.boolValue ? 1 : 0,
                new string[] {"Device Language", "First in List" + firstLanguage}, GUILayout.ExpandWidth(true)) == 1;
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();
        }
        ...
        void OnGUI_LanguageList()
        {
            ...
            mScrollPos_Languages = GUILayout.BeginScrollView(mScrollPos_Languages, EditorStyles.textArea,
                GUILayout.MinHeight(100), GUILayout.MaxHeight(Screen.height), GUILayout.ExpandHeight(false));
            ...
            for (int i = 0, imax = mProp_Languages.arraySize; i < imax; ++i)
            {
                ...
                if (TestButtonArg(eTest_ActionType.Button_Languages_ClearAll, i,
                    new GUIContent("Clear", "Clear all terms"), EditorStyles.toolbarButton,
                    GUILayout.ExpandWidth(false)))
                {
                    GUITools.DelayedCall(() => ClearAll(LanName));
                }
        ...
        private void ClearAll(string lanName)
        {
            if (mTestAction == eTest_ActionType.None && !EditorUtility.DisplayDialog("Confirm clear",
                "Are you sure you want to clear the selected terms", "Yes", "Cancel"))
                return;
            ...
            foreach (var termData in mLanguageSource.mTerms)
            {
                ...
                termData.SetTranslation(LanIndex, string.Empty);
            }
        }
