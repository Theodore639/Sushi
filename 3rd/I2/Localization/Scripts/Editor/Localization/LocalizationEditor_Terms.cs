...
namespace I2.Loc
{
    public partial class LocalizationEditor
    {
        ...
        void OnGUI_KeysList(bool AllowExpandKey = true, float Height = 300.0f, bool ShowTools = true)
        {
            ...
            bstyle.fontSize = 15;
            if (GUILayout.Button(
                new GUIContent("\u21bb", "Parse Scene and update terms list with missing and unused terms"), bstyle,
                GUILayout.Width(25)))
                EditorApplication.update += DoParseTermsInCurrentSceneAndScripts;
            if (GUILayout.Button(new GUIContent("\u21ea", "Refresh the translation of all Localize objects"), bstyle,
                GUILayout.Width(25)))
                CallLocalizeAll();
            ...
            mFlagsViewKeys = OnGUI_FlagToogle("Used", "Shows All Terms referenced in the parsed scenes", mFlagsViewKeys,
                (int) eFlagsViewKeys.Used);
            mFlagsViewKeys = OnGUI_FlagToogle("Not Used", "Shows all Terms from the Source that are not been used",
                mFlagsViewKeys, (int) eFlagsViewKeys.NotUsed);
            mFlagsViewKeys = OnGUI_FlagToogle("Missing", "Shows all Terms Used but not defined in the Source",
                mFlagsViewKeys, (int) eFlagsViewKeys.Missing);
            ...
            //--[ Keys List ]-----------------------------------------
            mScrollPos_Keys = GUILayout.BeginScrollView(mScrollPos_Keys, false, false, "horizontalScrollbar",
                "verticalScrollbar", EditorStyles.textArea,
                GUILayout.Height(
                    TermsListHeight) /*GUILayout.MinHeight(Height), GUILayout.MaxHeight(Screen.height), GUILayout.ExpandHeight(false)*/);
...
//				EditorGUILayout.HelpBox("Use (Tools\\Parse Terms) to find how many times each of the Terms are used", UnityEditor.MessageType.Info);
            ...
            if (ShowTools)
            {
                ...
                if (TestButton(eTest_ActionType.Button_AddSelectedTerms,
                    new GUIContent("Add Terms", "Add terms to Source"), "Button", GUITools.DontExpandWidth))
                    AddTermsToSource();
                if (TestButton(eTest_ActionType.Button_RemoveSelectedTerms,
                    new GUIContent("Remove Terms", "Remove Terms from Source"), "Button", GUITools.DontExpandWidth))
        ...
        static void doTranslate(int index, IReadOnlyList<TermData> data)
        {
            Timer.Schedule(GameManager.Instance, 3, () =>
            {
                if (index < data.Count)
                {
                    var term = data[index].Term;
                    LinneaTranslateLanguage(term, data[index], new Localize(), mLanguageSource);
                    Debug.Log("LinneaTerms index = " + index + ", key = " + term);

                    doTranslate(index + 1, data);
                }
                else
                {
                    Debug.Log("LinneaTerms 翻译完成 ！ 总数目 " + data.Count);
                }
            });
        }

        static void LinneaTranslateLanguage(string Key, TermData termdata, Localize localizeCmp, LanguageSourceData source)
        {
            ClearErrors();
        ...
        void OnGUI_KeyHeader(string sKey, string sCategory, string FullKey, int nUses, float YPosMin)
        {
            ...
            OnGUI_SelectableToogleListItem(new Rect(2, YPosMin + 3, 15, mRowSize), FullKey, ref mSelectedKeys,
                "OL Toggle");
            ...
            if (bKeyIsMissing)
            {
                ...
                GUI.Label(rect,
                    new GUIContent("", "This term is used in the scene, but its not localized in the Language Source"));
                MinX += rect.width;
            }
            else MinX += 3;
            ...
            if (mKeyToExplore == FullKey)
            {
                ...
                if (GUI.Button(rectKey, new GUIContent(sKey, EditorStyles.foldout.onNormal.background),
        ...
        void OnGUI_KeysList_AddKey()
        {
            ...
            if (bEnabled)
            {
                ...
                mTermsList_NewTerm = EditorGUILayout.TextField(mTermsList_NewTerm, EditorStyles.toolbarTextField,
                ...
                if (TestButton(eTest_ActionType.Button_AddTerm_InTermsList, "Create Key", "toolbarbutton",
        ...
        void OnGUI_SelectedCategories()
        {
            ...
            if (mSelectedCategories.Count() == 0)
                text = "Nothing";
            else if (mSelectedCategories.Count() == mParsedCategories.Count)
                text = "Everything";
            else
            ...
            if (GUILayout.Button(new GUIContent(text), "toolbarbutton", GUILayout.Width(100)))
            {
                ...
                for (int i = 0, imax = parsedList.Count; i < imax; ++i)
                {
                    ...
                    {
                        categoryRoot += "/";
                        var newCateg = (!category.Contains('/'))
                            ? category
                    ...
                    menu.AddItem(new GUIContent(displayName),
                        !string.IsNullOrEmpty(mSelectedCategories.FirstOrDefault(x => x.StartsWith(categoryRoot))),
                        () =>
                        {
        ...
        void LoadSelectedCategories()
        {
            ...
            {
                mSelectedCategories.Clear();
                mSelectedCategories.AddRange(
                    data.Split(",".ToCharArray(), System.StringSplitOptions.RemoveEmptyEntries));
            }
        }
        ...
        void OnGUI_Keys_ListSelection(int KeyListFilterID)
        {
            ...
            if (TestButton(eTest_ActionType.Button_SelectTerms_All,
                new GUIContent("All", "Selects All Terms in the list"), "toolbarbutton", GUILayout.ExpandWidth(false)))
            {
                mSelectedKeys.Clear();
                foreach (var kvp in mParsedTerms)
            ...
            if (GUILayout.Button(new GUIContent("None", "Clears the selection"), "toolbarbutton",
                GUILayout.ExpandWidth(false)))
            {
                mSelectedKeys.Clear();
            }
            ...
            if (TestButton(eTest_ActionType.Button_SelectTerms_Used,
                new GUIContent("Used", "Selects All Terms referenced in the parsed scenes"), "toolbarbutton",
                GUILayout.ExpandWidth(false)))
            {
                mSelectedKeys.Clear();
                foreach (var kvp in mParsedTerms)
            ...
            if (GUILayout.Button(new GUIContent("Not Used", "Selects all Terms from the Source that are not been used"),
                "toolbarbutton", GUILayout.ExpandWidth(false)))
            {
                mSelectedKeys.Clear();
                foreach (var kvp in mParsedTerms)
                    if (kvp.Value.Usage == 0 && ShouldShowTerm(kvp.Value.Term, kvp.Value.Category, kvp.Value.Usage))
            ...
            if (TestButton(eTest_ActionType.Button_SelectTerms_Missing,
                new GUIContent("Missing", "Selects all Terms Used but not defined in the Source"), "toolbarbutton",
                GUILayout.ExpandWidth(false)))
            {
                mSelectedKeys.Clear();
                foreach (var kvp in mParsedTerms)
                    if (!mLanguageSource.ContainsTerm(kvp.Key) &&
                        ShouldShowTerm(kvp.Value.Term, kvp.Value.Category, kvp.Value.Usage))
                        mSelectedKeys.Add(kvp.Key);
            }
            ...
            // Terms Filter
            {
                ...
                KeyList_Filter = GUITools.TextField(mKeyListFilterRect, KeyList_Filter, 255,
                    GUI.skin.GetStyle("ToolbarSeachTextField"), KeyListFilterID);
            }


            if (GUILayout.Button(string.Empty,
                string.IsNullOrEmpty(KeyList_Filter) ? "ToolbarSeachCancelButtonEmpty" : "ToolbarSeachCancelButton",
                GUILayout.ExpandWidth(false)))
            {
        ...
        static bool StringContainsFilter(string Term, string Filter)
        {
            ...
            Term = Term.ToLower();
            string[] Filters =
                Filter.ToLower().Split(";, ".ToCharArray(), System.StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            if (mTestAction == eTest_ActionType.None && !EditorUtility.DisplayDialog("Confirm delete",
                "Are you sure you want to delete the selected terms", "Yes", "Cancel"))
                return;
        ...
        List<GameObject> FindObjectsUsingKey(string Key)
        {
            ...
            for (int i = 0, imax = Locals.Length; i < imax; ++i)
            {
                ...
                if (localize == null || localize.gameObject == null ||
                    !GUITools.ObjectExistInScene(localize.gameObject))
                    continue;
